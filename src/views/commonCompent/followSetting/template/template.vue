		...
		<Card style="margin-top:10px">
			...
			<Row :gutter="20">
				...
				<div class="template_main_stepOne">
					...
					<Alert type="success">
						添加{{diseaseType}}显示对应的指标
					</Alert>
					<div class="template_main_disAdd">
						<Select :placeholder="`选择${diseaseType}获取对应的指标`" v-model="listDisID" filterable remote not-found-text="" :remote-method="remoteMethod_else" clearable @on-change="selectChange">
		...
		<Card style="margin-top:10px">
			...
			<Row class="setQuestion" :gutter="20">
				<Col span="6" class="questionList">
						...
						<Button type="primary" @click="addTemplate(0)">保存模板</Button>
								...
								<ul slot="content" v-for="item1 in item.questionTempleQuestionJumps">
									<li>
										<Row class="itemli">
											...
											<Row class="padleft40 mb5" v-if="item1.switchId==-2">
												<Col span="4" style="width:105px" class="lineheight32">无声音超次数跳转:</Col>
												<Col span="18" style="width:calc(100% - 105px)" class="textCenter">
												<Input placeholder="请填写无声音超次数跳转" v-model="item1.outRptSwitchID"></Input>
												</Col>
											</Row>
										</Row>
									</li>
  ...
  methods: {
    ...
    templateInfo() {
      API.followTemplate
        ...
        .then(res => {
           ...
           * 通用库赋值
           */
          this.tagCount = res.data.submoulds
            ? res.data.submoulds.split(";")
            : [];
        })
        .catch(error => {
          console.log(error);
      ...
      API.followTemplate
        ...
        .then(res => {
          ...
          res.data.forEach(item => {
            let flag = 0;
            for (let ite of this.stepThirdQuestion) {
              if (ite.title == item.questionName) {
                flag++;
              }
            }
            if (flag == 0) {
    ...
    addTag() {
      ...
      this.tagCount.forEach(item => {
        if (
          this.templateForm.addSubmoulds == item ||
    ...
    /*
		...
		*/
    addQuestion(item) {
      for (const ite of this.stepThirdQuestion) {
        if (ite.id == item.id) {
          return false;
        }
      }
      ...
      API.voiceSetting
        ...
        .then(res => {
          ...
          let maps = {
            ...
            targetId: item.targetId,
            questionTempleQuestionJumps: JSON.parse(
              JSON.stringify(this.switchArr)
            )
          };
          /**
           * 数据格式转化
    ...
    addTemplate(type) {
      ...
      API.followTemplate
        .addList({
          id: ids, //不传表示新增
          diseaseId: JSON.parse(
            JSON.stringify(this.templateForm.diseaseId)
          ).join(","), //疾病id
          ...
          status: 0, //0 正常 1 停用
          submoulds:
            this.tagCount.length > 1
              ? JSON.parse(JSON.stringify(this.tagCount)).join(";")
