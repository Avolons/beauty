				...
				<Tabs v-model="step" class="creatNotice_main_table">
					<TabPane label="标签一" name="step_one">
						...
						<Row class="creatNotice_main_search" :gutter="15">
							...
							<!--</Select>-->
							<Cascader  :data="cascardList" change-on-select  :render-format="changeFormat" @on-change='getDoctorList' style="width: 100%;"></Cascader>
							</Col>
							<Col span="6" v-if="nameShow">
							<span>
								医生：
							</span>
						...
						<div class="creatNotice_main_listBox">
							<Row class="creatNotice_main_search" :gutter="15">
								...
								<!--<Select :filterable="true" v-model="patParams.departmentId">-->
									<!--<Option v-for="item in departList" :value="item.id" :key="item.id">{{item.name}}</Option>-->
								<!--</Select>-->
								<Cascader  :data="cascardList" change-on-select   @on-change='handleDeparment' style="width: 100%;"></Cascader>
								</Col>
								<Col span="6">
								<span>
									疾病类型
...
//				this.departList = res.data;
		...
		getDoctorList(value) {
            if(value){
                this.departId = value[value.length-1]
            }else{
                this.departId = '';
            }
