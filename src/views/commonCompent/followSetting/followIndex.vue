...
<template>
	<Row class="followIndex">
		...
		<Row class="followIndex_main_search" :gutter="15">
			...
			<Select v-model="IndexSearch.diseaseId" filterable remote not-found-text="" :remote-method="remoteMethod2" :label-in-value="true" clearable @on-change="selectChange" :placeholder="`请输入${diseaseType}`">
...
export default {
	...
	methods: {
		...
		*/
		clearAll() {
			this.formItem = {
				...
				remark: "",                    //备注
				isCheck: 1 ,                    //是否需要考核：0，是；1，否
				mouldType: this.diseaseType == "疾病类型" ? 0 : 1
			};
			this.labelobj = [];
			this.diseaseList = [];
			this.tagCount = [];
		...
		*/
		addModel(name) {
			if (this.formItem.type == "select" && this.formItem.optionValues.length == 0) {
			...
			this.$refs[name].validate((valid) => {
				if (valid) {
					...
					sendData.optionValues = sendData.type == "select" ? sendData.optionValues.join(',') : "";
