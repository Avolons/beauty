				...
				<Tabs v-model="step" class="creatNotice_main_table">
					<TabPane label="标签一" name="step_one">
						...
						<Row class="creatNotice_main_search" :gutter="15">
							...
							<Cascader  :data="cascardList" change-on-select @on-change='getDoctorList'  :render-format="changeFormat" style="width: 100%;"></Cascader>
							...
							<Select :filterable="true" @on-change="doctorChange" v-model="doctorobj">
						...
						<div class="creatNotice_main_listBox">
							<Row class="creatNotice_main_search" :gutter="15">
								...
								<!--<Select v-model="patParams.departmentId">-->
									<!--<Option v-for="item in departList" :value="item.id" :key="item.id">{{item.name}}</Option>-->
								<!--</Select>-->
								<Cascader  :data="cascardList" change-on-select :render-format="changeFormat"  @on-change='getDoctorLTwo' style="width: 100%;"></Cascader>
								</Col>
								<Col span="6">
								<span>
									疾病类型
...
export default {
	...
	methods: {
        ...
        getDoctorLTwo(value){
            if(value){
                this.patParams.departmentId = value[value.length-1]
			}
		},
        /**
         * 数据格式化
...
//
		 ...
		 * 获取医生列表
		 */
		getDoctorList(value, selectedData) {
