	...
	<Row class="way" :gutter="10">
		...
		<Card>
			...
			<Form ref="wayForms" :model="baseData" :label-width="80" :rules="validate.followAction" class="way_main_wayForm">
				...
				<FormItem label="科室类别" prop="departmentId">
					...
					<!--</Select>-->
					<Cascader ref="cascader" filterable v-model="baseDepart" :data="cascardList" change-on-select :render-format="changeFormat"   @on-change='handleDeparment' style="width: 100%;"></Cascader>
				</FormItem>
				<FormItem label="方案类型" prop="activeType">
					<RadioGroup v-model="baseData.activeType">
						<Radio v-show="diseaseType=='疾病类型'" label="0">随访</Radio>
						<Radio v-show="diseaseType=='疾病类型'"  label="1">通知</Radio>
						<Radio v-show="diseaseType=='疾病类型'"  label="3">出院随访</Radio>
						<Radio v-show="diseaseType=='疾病类型'"  label="4">入院通知</Radio>
						<Radio v-show="diseaseType!='疾病类型'"  label="5">体检</Radio>
					</RadioGroup>
				</FormItem>
				<FormItem :label="diseaseType" prop="diseaseId">
					<Select :label="labelobj" v-model="baseData.diseaseId" not-found-text="" multiple filterable remote :remote-method="autoSearch_base" :label-in-value="true" :placeholder="`请输入${diseaseType}`">
		...
		<Card style="margin-top:10px">
			...
			<Form :label-width="80" class="way_main_wayForm">
				<FormItem :label="diseaseType">
					<Select v-model="selectDisTemp" not-found-text="" multiple filterable remote :remote-method="autoSearch_tem" @on-change="selectChange" :label-in-value="true" :placeholder="`搜索${diseaseType}`">
			...
			<ul class="way_main_planList">
				<li class="way_main_planSingle" v-for="(item,index) in showList" :key="item.id">
					...
					<Row>
						...
						<Col span="12" class="lineheight32 way_main_commonTemTitle">
						<strong>{{item.name}}</strong>
						</Col>
						<Col span="6" class="lineheight32 ">
						<Button size="small" style="float:right;margin-left:10px" type="info" icon="ios-cloud-download" @click="downloadTemp(index,item)"></Button>
		...
		<Modal v-model="seeModal" :styles="{left: '20px'}" width='600' title="预览计划">
...
/* let localDepart=JSON.parse(localStorage.getItem("departmentList")); */
export default {
  props: {
    diseaseType: {
      type: String,
  ...
  data() {
    return {
      ...
      baseData: {
        ...
        activeType: this.diseaseType=="疾病类型"?"0":"5", //方案类型：0代表随访，1代表通知
        status: 0, //状态：0，启用；1，禁用
        visitStartTime: "",
        mouldType: this.diseaseType == "疾病类型" ? 0 : 1 //类型：0，普通随访；1，体检随访（必须）
      },
      labelobj: [], //疾病多选初始化名称
      cascardList: /* localDepart?localDepart: */[], //科室列表
      diseaseList: [], //疾病列表
      /**
       * 最终需要提交数据的列表部分
       */
  ...
  methods: {
    ...
    handleDeparment(value) {
      if (value) {
        this.baseData.departmentId = value[value.length - 1];
      } else {
        this.baseData.departmentId = "";
      }
    },
    /** 
		模板下载功能
		 */
    downloadTemp(index, item) {
      let aLink = document.createElement("a");
      let linkurl= API.followWay.specialExport+'?schemeId='+this.followId+'&templeId='+item.id+'&sort='+index;
      aLink.href = linkurl;
          if (document.all) {
            aLink.click();
          } else {
    ...
    submitCeshi() {
      ...
      API.FollowBussiness.patCeshi(param)
        .then(res => {
      ...
      API.followTemplate
        ...
        .then(
          res => {
            this.temList[
              index
            ].questionTemples.questionSchemeWavs = this.differData(
              res.data,
              this.temList[index].questionTemples.questionSchemeWavs
            );
    ...
    differData(newdata, old) {
      for (let item of newdata) {
        for (let ite of old) {
          if (item.questionId == ite.questionId && item.questionIdXml == ite.questionIdXml) {
            /**
             * 循环之前初始默认节点
             */
    ...
    saveChange(type = 0) {
      ...
      for (let item of this.showList) {
        ...
        if (flags > 0) {
          copyItem.questionTempleFrequency.firstday = 0;
        }
        for (let ite of item.questionTemples.questionSchemeWavs) {
      ...
      if (this.baseData.activeType > 3) {
        for (let item of sendData.questionTemples) {
          if (item.questionTempleFrequency.firstday >= 0) {
            item.questionTempleFrequency.firstday =
              0 - item.questionTempleFrequency.firstday;
          }
        }
      }
        ...
        .then(res => {
          ...
          setTimeout(() => {
            if (this.diseaseType == "疾病类型") {
              ...
              this.$router.push("/physical/noticeSetting/project");
            }
          }, 500);
        })
        .catch(error => {});
      ...
      for (const item of this.editList) {
        ...
        for (const ite of item.questionSchemeWavs) {
          if (arr.length == 0) {
            ...
            let flag = 0;
            for (const it of arr) {
              if (it.questionIdXml == ite.questionIdXml) {
        ...
        item.questionTemples.questionTempleFrequency.firstday = Math.abs(
          item.questionTemples.questionTempleFrequency.firstday
        );
        /**
         * 删除多余的属性
         */
        ...
        .then(res => {
          ...
          let getArray = point => {
            for (const iterator of res.data) {
              if (iterator.id == point) {
                this.baseDepart.unshift(point);
                if (iterator.parentId) {
                  getArray(iterator.parentId);
                }
                break;
              }
            }
          };
          getArray(this.baseData.departmentId);
          this.cascardList = JSON.parse(JSON.stringify(copyData));
          if(edit){
            this.getvoiceList();
          }
          /* this.$refs.cascader.updateSelected(true); */
          /* localStorage.setItem("departmentList",JSON.stringify(copyData)); */
        })
        .catch(error => {});
    }
  },
